...
- **Pisahkan logika bisnis dari route handler.** Fungsi seperti `smart-split.ts`, `trust-score-engine.ts`, dan `economic-impact-calc.ts` harus jadi fungsi murni di `/backend/src/lib`, dipanggil dari route Express — bukan ditulis langsung di dalam handler, supaya bisa diuji terpisah (unit test tanpa perlu server berjalan).
- **Pengecualian: Logic perhitungan fair-share diimplementasikan sebagai PostgreSQL RPC function.** Logic perhitungan `calculate_fair_share` sengaja diimplementasikan sebagai PostgreSQL RPC function di `backend/db/02_fair_share_rpc.sql` (bukan fungsi murni TypeScript di `/backend/src/lib`), dengan alasan: butuh row locking (`FOR UPDATE`) dan atomicity transaksi yang lebih aman dilakukan native di level database daripada di-reimplementasi manual di Node.js. Route handler `POST /api/fair-share/:order_id/calculate` di `fairShare.ts` hanya memanggil RPC ini tanpa logic perhitungan tambahan.
