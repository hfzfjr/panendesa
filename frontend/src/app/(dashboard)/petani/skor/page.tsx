...
import { Target, CalendarCheck, Leaf, Sparkles, ChevronRight, Apple, Sprout, Leaf as LeafIcon, ChevronLeft, Award } from "lucide-react";
...
export default function TrustScorePage() {
  ...
  useEffect(() => {
    const fetchTrustScore = async () => {
      ...
      try {
        const response = await apiClient.getTrustScorePetani(user.user_id);
        if (response.success && response.data) {
          setTrustScore(response.data);
        } else {
          setError({ message: response.error || 'Gagal mengambil skor konsistensi' });
        }
      } catch (err) {
    ...
    return (
      <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-5 bg-gray-200 rounded-lg h-80 animate-pulse" />
  ...
  if (!trustScore) {
    return (
      <div className="p-4 md:p-8 max-w-7xl mx-auto">
        <ErrorState code={404} message="Data skor tidak ditemukan" onRetry={() => window.location.reload()} />
      </div>
    );
  }
  return (
        ...
        <div className="lg:col-span-5 bg-white rounded-lg border border-gray-100 shadow-sm p-8 flex flex-col items-center justify-center relative overflow-hidden">
          ...
          <div className="relative w-64 h-64 flex items-center justify-center mb-6">
            ...
            <div className="absolute flex flex-col items-center justify-center text-center">
              <span className="text-5xl font-black text-primary-dark">{trustScore.skor_konsistensi}</span>
              <span className="text-xs font-bold text-gray-500 tracking-widest mt-1">SKOR TOTAL</span>
            </div>
          </div>

          <div className={`bg-success/10 px-4 py-1.5 rounded-full font-bold text-sm mb-4 ${getTrustScoreBadge(trustScore.skor_konsistensi).className}`}>
            {getTrustScoreBadge(trustScore.skor_konsistensi).label}
          </div>

          <p className="text-gray-600 text-center font-medium text-sm leading-relaxed px-4">
            Berdasarkan <span className="font-bold text-gray-900">{trustScore.jumlah_transaksi_dihitung}</span> transaksi<br />
            yang telah dihitung.
          </p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[500px]">
              ...
              <tbody className="divide-y divide-gray-50">
                {trustScore.riwayat.map((riwayat, idx) => (
                  <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-gray-900 text-sm">Komoditas #{riwayat.stok_estimasi_id}</span>
                      </div>
                    </td>
                    <td className="py-4 text-sm">
                      <div className="flex items-center gap-2">
                        <span className="text-gray-500">{riwayat.estimasi_kg}kg</span>
                        <ChevronRight className="w-3 h-3 text-gray-300" />
                        <span className="font-bold text-gray-900">{riwayat.realisasi_kg}kg</span>
                      </div>
                    </td>
