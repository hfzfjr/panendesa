...
export default function CheckoutPage() {
  ...
  // Form state - from query parameters
  const [formData, setFormData] = useState({
    komoditas_id: 1,
    jumlah_diminta_kg: 500,
    desa_id_prioritas: 1,
  });
  ...
  useEffect(() => {
    ...
    if (desaId && komoditasId && jumlah) {
      setFormData({
        komoditas_id: parseInt(komoditasId),
        jumlah_diminta_kg: parseInt(jumlah),
        desa_id_prioritas: parseInt(desaId),
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiClient.postOrder(
        formData.komoditas_id,
        formData.jumlah_diminta_kg,
        formData.desa_id_prioritas
      );

      if (response.success && response.data) {
        setToastMessage(`Pesanan #${response.data.order_id} berhasil dibuat! Status: ${response.data.status}`);
        ...
        setToastMessage(response.error || 'Gagal membuat pesanan');
        setToastType('error');
        setShowToast(true);
      }
    } catch (err) {
      setError('Terjadi kesalahan saat membuat pesanan');
      setToastMessage('Terjadi kesalahan saat membuat pesanan');
      setToastType('error');
      setShowToast(true);
    } finally {
      setLoading(false);
    ...
    <>
      ...
      <main className="max-w-7xl mx-auto px-4 lg:px-8 py-6 md:py-12 pb-28">
        ...
        <div className="flex items-center gap-3 mb-6 md:mb-8">
          <Link href="/pembeli/marketplace" className="p-2 hover:bg-gray-100 rounded-full transition-colors -ml-2">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-gray-500 text-sm md:text-base hidden sm:block">Konfirmasi pesanan Anda sebelum submit</p>
          </div>
        </div>

        {/* Show error if accessed without parameters */}
        {!hasValidParams && (
          <div className="bg-white rounded-2xl border border-gray-200 p-8 md:p-12 text-center">
            <Info className="w-16 h-16 text-gray-300 mx-auto mb-4" />
            <h3 className="font-bold text-gray-900 text-lg mb-2">Pilih Komoditas Terlebih Dahulu</h3>
            <p className="text-gray-500 text-sm mb-6 max-w-sm mx-auto">
              Silakan pilih komoditas dari marketplace sebelum melakukan checkout.
            </p>
        ...
        {hasValidParams && (
          <>
            <div className="inline-flex items-center gap-2 bg-gray-100/80 px-4 py-2 rounded-lg text-sm text-gray-600 mb-6">
              ...
              Pastikan rincian pesanan Anda sudah sesuai sebelum submit.
            </div>
                  ...
                  <div className="hidden lg:block space-y-3 mt-4">
                    <form onSubmit={handleSubmit}>
                      <Button
                        type="submit"
                        disabled={loading}
                        className="w-full h-14 rounded-xl bg-primary-dark hover:bg-primary font-bold text-[15px] flex justify-between items-center px-6 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      >
            ...
            <div className="lg:hidden fixed bottom-16 left-0 w-full bg-white border-t border-gray-200 p-4 z-40 shadow-sm">
              ...
              <form onSubmit={handleSubmit}>
                <Button
                  type="submit"
                  disabled={loading}
                  className="w-full h-12 rounded-xl bg-primary-dark hover:bg-primary font-bold text-[15px] flex justify-center items-center transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loading ? 'Memproses...' : 'Konfirmasi Pesanan'}
                </Button>
              </form>
