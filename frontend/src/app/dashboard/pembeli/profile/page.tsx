  ...
  return (
    <main className="max-w-2xl mx-auto px-4 lg:px-8 py-6 md:py-12 pb-28 min-h-screen">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6 flex items-center gap-5 relative overflow-hidden">
        {/* Background accent */}
        <div className="absolute top-0 right-0 w-32 h-32 bg-[#1B5E20]/5 rounded-full -translate-y-1/2 translate-x-1/3"></div>
        ...
        <div className="relative z-10">
          ...
          <span className="inline-flex items-center bg-[#1B5E20]/10 text-[#1B5E20] px-3 py-1 rounded-full text-xs font-bold">
            Member Aktif
          </span>
        </div>
      </div>
        ...
        {menuItems.map((item, index) => {
          ...
          return (
            <Link 
              key={index} 
              href={item.href}
              className={`flex items-center justify-between p-4 md:p-5 hover:bg-gray-50 transition-colors ${
                index !== menuItems.length - 1 ? "border-b border-gray-100" : ""
              }`}
            >
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-[#F8FAF6] flex items-center justify-center shrink-0">
