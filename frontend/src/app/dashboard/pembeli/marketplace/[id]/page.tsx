...
export default function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main className="max-w-7xl mx-auto pb-24 md:pb-12 min-h-screen bg-gray-100 md:bg-transparent">
      ...
      <Link href="/dashboard/pembeli/marketplace" className="md:hidden fixed top-4 left-4 z-50 p-2 bg-black/30 backdrop-blur-sm text-white hover:bg-black/40 rounded-full transition-colors shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-0 md:gap-8 lg:gap-10 md:py-8 lg:px-8">
            ...
            <div className="flex gap-3 px-4 md:px-0 py-4 md:py-4 overflow-x-auto scrollbar-hide bg-white md:bg-transparent">
              {productInfo.images.map((img, idx) => (
                <button
                  ...
                  className={`w-16 h-16 md:w-20 md:h-20 shrink-0 rounded-xl overflow-hidden border-2 transition-all ${activeImage === idx ? 'border-[#1B5E20]' : 'border-transparent hover:border-gray-300'}`}
                >
        ...
        <div className="lg:col-span-5 md:col-span-1 flex flex-col gap-2 md:gap-6 bg-transparent">
          ...
          {/* Desktop Back Button */}
          <Link href="/dashboard/pembeli/marketplace" className="hidden md:inline-flex items-center gap-2 text-gray-500 hover:text-[#1B5E20] font-bold transition-colors">
          ...
          <div className="bg-white p-4 md:p-0 md:bg-transparent rounded-none md:rounded-none shadow-sm md:shadow-none">
            ...
            <div className="text-3xl font-extrabold text-[#1B5E20] mb-3">
          ...
          <div className="bg-white p-4 md:p-6 md:rounded-2xl md:border border-gray-200 shadow-sm md:shadow-none">
            <Link href="#" className="flex items-center justify-between group">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-1.5 mb-0.5">
                    ...
                    <h3 className="font-bold text-gray-900 group-hover:text-[#1B5E20] transition-colors">{productInfo.farmer}</h3>
                  </div>
              ...
              <ChevronRight className="w-5 h-5 text-gray-400 group-hover:text-[#1B5E20] transition-colors" />
            </Link>
          </div>
        ...
        <div className="lg:col-span-3 lg:block">
          <div className="fixed bottom-0 left-0 right-0 md:sticky md:top-24 bg-white md:rounded-2xl md:border border-t border-gray-200 p-4 shadow-[0_-4px_12px_rgba(0,0,0,0.05)] md:shadow-sm z-40">
            ...
            <div className="flex items-center justify-between mb-4 md:mb-5">
              <div className="flex items-center border border-gray-300 rounded-xl overflow-hidden h-10 md:h-11">
                ...
                <button
                  ...
                  className="w-10 h-full flex items-center justify-center text-[#1B5E20] hover:bg-green-50 transition-colors"
                >
              ...
              <div className="text-right">
                ...
                <p className="font-bold text-gray-900 text-sm"><span className="text-[#1B5E20]">{productInfo.stock}</span> {productInfo.unit}</p>
              </div>
            </div>
            ...
            <div className="items-center gap-2 text-xs text-gray-600 mb-5 bg-gray-50 p-2.5 rounded-lg border border-gray-100 hidden md:flex">
              <Truck className="w-4 h-4 shrink-0 text-[#1B5E20]" />
              <span className="leading-snug">Pengiriman langsung dari Desa Sukamaju</span>
            </div>

            {/* Action Buttons */}
            <div className="flex gap-2 md:gap-3">
              <Button variant="custom" className="flex-none w-12 md:w-auto md:flex-1 md:px-0 h-11 md:h-12 rounded-xl bg-white border-2 border-[#1B5E20] text-[#1B5E20] hover:bg-green-50 font-bold transition-colors flex items-center justify-center">
                <ShoppingCart className="w-5 h-5 md:mr-2" />
                <span className="hidden md:inline">Keranjang</span>
              </Button>
              <Link href="/dashboard/pembeli/checkout" className="flex-1">
                <Button className="w-full h-11 md:h-12 rounded-xl bg-[var(--color-primary-dark)] hover:bg-[var(--color-primary)] text-white font-bold text-sm md:text-base transition-all shadow-md">
                  Beli Langsung
                </Button>
              </Link>
            </div>
