    ...
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-6 pb-32 md:pb-8">
      ...
      <form className="space-y-8" onSubmit={(e) => e.preventDefault()}>
        <div className="bg-white rounded-3xl p-6 border border-gray-200 shadow-sm">
          ...
          <button type="button" className="w-full border-4 border-dashed border-[#1B5E20]/30 hover:border-[#1B5E20] bg-green-50/50 hover:bg-green-50 rounded-2xl h-48 md:h-64 flex flex-col items-center justify-center gap-4 transition-colors group">
            <div className="w-16 h-16 bg-[#1B5E20] text-white rounded-full flex items-center justify-center group-hover:scale-110 transition-transform shadow-lg">
              <Camera className="w-8 h-8" />
            </div>
            <div className="text-center">
              <span className="block font-bold text-[#1B5E20] text-lg md:text-xl">Ambil Foto Langsung</span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {["Bibit", "Tumbuh (Vegetatif)", "Mulai Berbunga", "Mulai Berbuah"].map((item) => (
              <button 
                ...
                className={`py-4 px-2 font-bold text-base md:text-lg rounded-2xl border-2 transition-all ${fase === item ? 'bg-[#1B5E20] text-white border-[#1B5E20] shadow-md' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}
              >
                {item}
              </button>
            ))}
        ...
        <div className="bg-white rounded-3xl p-6 border border-gray-200 shadow-sm">
          ...
          {kondisi === "Bermasalah" && (
            <div className="mt-6 p-5 bg-red-50 border-2 border-red-200 rounded-2xl animate-fade-in">
        ...
        <div className="bg-white rounded-3xl p-6 border border-gray-200 shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-4">
            <label className={`flex items-center justify-between p-5 rounded-2xl border-2 cursor-pointer transition-colors ${isDisiram ? 'border-[#1B5E20] bg-green-50' : 'border-gray-200 bg-white'}`}>
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${isDisiram ? 'bg-[#1B5E20] text-white' : 'bg-gray-100 text-gray-500'}`}>
                  <Droplets className="w-6 h-6" />
                </div>
                <span className={`font-bold text-lg md:text-xl ${isDisiram ? 'text-[#1B5E20]' : 'text-gray-700'}`}>Saya Menyiram Lahan</span>
              </div>
              <input 
                type="checkbox" 
                checked={isDisiram}
                onChange={() => setIsDisiram(!isDisiram)}
                className="w-8 h-8 rounded border-gray-300 text-[#1B5E20] focus:ring-[#1B5E20] cursor-pointer"
              />
            </label>
            <label className={`flex items-center justify-between p-5 rounded-2xl border-2 cursor-pointer transition-colors ${isDipupuk ? 'border-[#1B5E20] bg-green-50' : 'border-gray-200 bg-white'}`}>
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${isDipupuk ? 'bg-[#1B5E20] text-white' : 'bg-gray-100 text-gray-500'}`}>
                  <FlaskConical className="w-6 h-6" />
                </div>
                <span className={`font-bold text-lg md:text-xl ${isDipupuk ? 'text-[#1B5E20]' : 'text-gray-700'}`}>Saya Memberi Pupuk</span>
              </div>
              <input 
                type="checkbox" 
                checked={isDipupuk}
            ...
            <label className={`flex items-center justify-between p-5 rounded-2xl border-2 cursor-pointer transition-colors ${isDibersihkan ? 'border-[#1B5E20] bg-green-50' : 'border-gray-200 bg-white'}`}>
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${isDibersihkan ? 'bg-[#1B5E20] text-white' : 'bg-gray-100 text-gray-500'}`}>
                  <Shovel className="w-6 h-6" />
                </div>
                <span className={`font-bold text-lg md:text-xl ${isDibersihkan ? 'text-[#1B5E20]' : 'text-gray-700'}`}>Membersihkan Lahan/Gulma</span>
              </div>
              <input 
                type="checkbox" 
        ...
        <div className="fixed bottom-0 left-0 right-0 md:static bg-white md:bg-transparent border-t md:border-none border-gray-200 p-4 md:p-0 z-40 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.1)] md:shadow-none pb-safe">
          <div className="max-w-4xl mx-auto flex gap-3">
            <button 
              type="button" 
              className={`w-full flex items-center justify-center gap-2 py-4 px-10 font-black text-white text-lg rounded-2xl transition-all shadow-xl ${kondisi ? 'bg-[#1B5E20] hover:bg-green-800' : 'bg-gray-300 cursor-not-allowed shadow-none'}`}
              disabled={!kondisi}
            >
