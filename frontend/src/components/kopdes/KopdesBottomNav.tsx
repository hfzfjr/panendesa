...
import { LayoutDashboard, Inbox, Package, ShoppingCart } from "lucide-react";
...
export function KopdesBottomNav() {
  ...
  const navItems = [
    ...
    { name: "Intake", href: "/kopdes/penerimaan", icon: Inbox },
    { name: "Stok", href: "/kopdes/stok", icon: Package },
    { name: "Pesanan", href: "/kopdes/pesanan", icon: ShoppingCart },
  ];
        ...
        return (
          ...
          >
            <div className={`p-1.5 rounded-full transition-colors ${isActive ? 'bg-green-50' : 'bg-transparent'}`}>
              <item.icon
                className={`w-6 h-6 ${isActive ? 'text-primary-dark' : 'text-gray-400'}`}
                strokeWidth={isActive ? 2.5 : 2}
              />
            </div>
            <span
              className={`text-[10px] font-bold ${isActive ? 'text-primary-dark' : 'text-gray-400'}`}
            >
              {item.name}
            </span>
          </Link>
