  ...
  const results = [
    {
      ...
      scoreColor: "bg-primary-dark", // Dark green
      gradeColor: "bg-primary", // Green
      stockVal: 1200,
      stockEst: 1500,
      price: "15.000",
    ...
    },
    {
      ...
      scoreColor: "bg-neutral-200 text-gray-700", // Greyish
      gradeColor: "bg-neutral-600", // Brown
      stockVal: 850,
      stockEst: 2000,
      price: "13.800",
      ...
      {results.map((item) => {
        ...
        return (
          <div key={item.id} className="bg-white rounded-2xl border border-gray-200 p-4 md:p-6 shadow-sm flex flex-col md:flex-row gap-6 items-start hover:shadow-md transition-shadow">
            ...
            {/* Image Container */}
            <Link href={`/pembeli/marketplace/${item.id}`} className="w-full md:w-48 h-48 rounded-xl overflow-hidden relative shrink-0 block group">
            ...
            <div className="flex-1 w-full flex flex-col justify-between h-full min-h-48">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start mb-4 gap-2">
                <div>
                  <Link href={`/pembeli/marketplace/${item.id}`}>
                    <h3 className="text-xl font-bold text-gray-900 hover:text-primary-dark transition-colors">{item.village}</h3>
                  </Link>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center sm:items-end mt-auto pt-4 border-t border-gray-100 gap-4">
                ...
                <Link href={`/pembeli/marketplace/${item.id}`} className="w-full sm:w-auto">
